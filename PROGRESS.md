...
**⚠️ Deploy needed (nothing shipped this session — code only):** (1) apply **`025_puja_bookings.sql`** + **`026_puja_slots.sql`** (Supabase SQL editor); (2) **redeploy `create-order` + `verify-payment`** (create-order enforces the slot cutoff + reads the slot row) and **deploy `puja-admin`**; (3) set Edge secrets **`OWNER_EMAILS`** (= `aayush.kumarsep1984@gmail.com`, for the admin) and optionally **`OWNER_NOTIFY_URL`**. Until `puja-admin` is deployed the admin screen shows "Edge Function returned a non-2xx" for the bookings list (slot editor still shows the config fallback); until `026` is applied the countdown uses the config default. **Open (non-blocker):** refund path + `refunded` status (manual via Razorpay), GST/invoice + booking T&Cs in `app/legal/`, `preferred_date` captured but no real slot inventory, Kundli→Pitra-Dosha detection funnel (subtle, deferred to Phase 2).
