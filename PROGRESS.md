...
**Next-slot countdown (added later, same day):** the puja runs on a scheduled slot (`NEXT_SLOT` in `config/pujas.ts` — **3 Oct 2026**), with **bookings closing 3 days before** (end of 30 Sep IST). New `components/SlotCountdown.tsx` shows "Next Puja · <date>" + a live ticking "Bookings close in [d][h][m][s]" pill row on the detail page (compact one-liner on the listing card). Once the cutoff passes, Book Now/Pay are **blocked** ("Bookings for this slot are closed — next slot opening soon") — enforced client-side AND in `create-order` (`slot_closed`, 409). Each booking stamps `puja_bookings.preferred_date` = the slot date. To roll to the next slot, the owner edits the five `NEXT_SLOT` fields (+ the `PUJA_SLOT` mirror in `create-order`).

**In-app Owner Admin (added later, same day):** since the owner has no admin panel, the slot is now **DB-driven** (`puja_slots` table, migration `026`) so it's editable from the app with NO redeploy. New **`app/puja-admin.tsx`** (route `/puja-admin`, hidden behind a Settings → OWNER row shown only to owner accounts) lets the owner: set the next slot (date + cutoff days) and view/manage all paid bookings (mark in-progress/completed/refunded). Backed by new Edge Function **`puja-admin`** (owner-gated by the `OWNER_EMAILS` secret; service-role reads all bookings / upserts the slot). Client gate: `config/owner.ts` `OWNER_EMAILS` — the app-login Google account is **`aayush.kumarsep1984@gmail.com`** (double-a; distinct from the Claude account). `lib/pujaSlot.ts` fetches the slot (DB → config fallback) + formats the date; `create-order` reads the slot row for the cutoff/preferred_date. The `SlotCountdown` + booking guard now read the DB slot. GOTCHA: adding the new `/puja-admin` route file needs a **Metro restart** (route manifest is cached) — a plain reload shows "Unmatched Route".

**✅ DEPLOYED (2026-07-17, user-authorized):** the whole puja feature is now live. Migrations **`025` + `026` + `027`** applied (via CLI `db push`; see gotcha below). Edge Functions deployed: **`create-order`**, **`verify-payment`**, **`puja-admin`**. Secret **`OWNER_EMAILS`** = `aayush.kumarsep1984@gmail.com,pooja.singh72@gmail.com,santosh.singh077@gmail.com` (three owners; also in `config/owner.ts`). `OWNER_NOTIFY_URL` still unset (optional — owner checks the admin instead). Razorpay in test mode. Verified on device: admin loads, "No bookings" (empty), slot editor shows the DB slot.

**Migration gotchas hit this session (for next time):** (1) the two **`024`** files share version `024`, which is only recorded once in history → `db push` errors with `duplicate key … schema_migrations_pkey (024)`. Fix: `migration repair --status applied 024`, then temporarily **move `024_report_pages.sql` out of `supabase/migrations/`** (keep exactly one `024` file so the tracker matches remote), `db push`, then move it back. (2) The live `puja_bookings` predated the `gotras` column (created from an earlier `025`; `create table if not exists` skipped the update), so `puja-admin`'s `select … gotras` 400'd → the admin's "Edge Function returned non-2xx". Fixed by **`027_puja_bookings_gotras.sql`** (`add column if not exists gotras`). Diagnosed by probing PostgREST directly: `GET /rest/v1/puja_bookings?select=<cols>` returns 400 naming a missing column. (`supabase db dump` needs Docker — unavailable here.) **Open (non-blocker):** refund path + `refunded` status (manual via Razorpay), GST/invoice + booking T&Cs in `app/legal/`, `preferred_date` captured but no real slot inventory, Kundli→Pitra-Dosha detection funnel (subtle, deferred to Phase 2).
...
- **App icons + splash regenerated from `logo.svg`** (I strip its background rectangle → transparent real logo, then place at apt sizes). All in `assets/`: `icon.png` (0.66 fill on `#0D0D1A`), `favicon.png`, `android-icon-foreground.png` (**0.56 fill** — adaptive foregrounds get ~1.5× launcher zoom, so 0.70 rendered edge-to-edge/"too big"), `android-icon-background.png` (solid `#0D0D1A`), `android-icon-monochrome.png` + `notification-icon.png` (white ऋ silhouette — derived from the SVG's **luminance**, NOT the mark's alpha, whose glow fills the letter counters into a blob), `splash-icon.png` (native splash = mark on dark). `app.json`: added `adaptiveIcon.monochromeImage`; splash uses `resizeMode:contain` + mark. **Native splash can't be the full nebula image** — Android 12+ only allows a centred masked icon on a bg colour, so the full static composition is delivered by the JS animated splash instead.
- **`components/AnimatedSplash.tsx` rebuilt → "The First Beat"** (post-native-splash JS overlay in `app/_layout.tsx`): nebula fades in → gold bindu seeds & blooms → ऋ mark ignites → **two staggered rings pulse outward (the beat)** + a mark kick → "Ritham" wordmark rises → fades to app. Pure RN Animated, no new deps. Uses `assets/splash/{mark,bindu,bg}.webp` (alpha-preserving WebP, ~167 KB total vs 4.5 MB PNG). Replaced the old orbit-ring splash; removed `logo-ring.png`/`logo-center.png`.
- **Home header logo removed** (`app/(tabs)/index.tsx`) — header is now just the "Ritham" wordmark; `logo-transparent.png` + its tile/styles deleted.
- **Banner art PNG→WebP** (`assets/banners|horoscope|dream|numerology/*` → `.webp`, ~95% smaller, same aspect ratios; require paths in `constants/appArt.ts` + inline updated) and **`HeroBanner` gained `blend`/`vignette`** edge-fade looks (art dissolves into the page instead of a bordered card) — applied to panchang/muhurat/sadesati/vakri/dream/horoscope (blend) and numerology (vignette). Kept the `ImageBackground` layout (an earlier absolute-image rewrite broke aspect ratios). **Temple images are real photos** (see `AI_ART_PROMPTS.md`), left as PNG.
- **Removed** the paywall relic image (`Paywall.tsx`) and chat guru-seated hero (`chat.tsx`) — looked out of place.
- **Device build note:** icon/splash/notification are compiled into the native binary → need a rebuild to show. `expo run:android` ALONE kept the OLD cached icons (the stale committed `android/` res); you must run **`npx expo prebuild -p android --clean`** first to re-bake icons from the new assets, then `expo run:android`. `android/` is gitignored (CNG), so none of this is in git. Wireless ADB: `adb connect 192.168.1.14:5555` + `adb reverse tcp:8081 tcp:8081`.

---

## Prior Session (2026-07-15, evening) — Reports tab rebrand (AI emblem art) + My Reports page
