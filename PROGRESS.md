...
**⚠️ Deploy needed (nothing shipped this session — code only):** (1) apply **`025_puja_bookings.sql`** (Supabase SQL editor — creates `puja_bookings` + RLS, widens `payment_orders.kind` to include `'puja'`); (2) **redeploy `create-order` + `verify-payment`** (create-order now also enforces the slot cutoff); (3) set Edge secret **`OWNER_NOTIFY_URL`** (else bookings are only visible in the dashboard). **Open (non-blocker):** refund path + `refunded` status (manual via Razorpay), GST/invoice + booking T&Cs in `app/legal/`, `preferred_date` captured but no real slot inventory, Kundli→Pitra-Dosha detection funnel (subtle, deferred to Phase 2).
