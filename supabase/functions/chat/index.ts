  ...
  try {
    ...
    if (sessionId) {
      ...
      if (claimedFree) {
        ...
        if (deviceHash && deviceReserved) await admin.from('device_free_trials').delete().eq('device_hash', deviceHash);
        // If THIS device already burned its free minute under another account
        // (delete-and-recreate / reinstall / shared device), this user can never earn
        // it here. Stamp the per-account flag so the client stops offering a "first
        // minute free" it can't grant (which would just bounce to the paywall). Only
        // on a real device block (deviceHash present but reservation lost) — never when
        // the id is simply absent, which is a soft, retryable deny.
        if (deviceHash && !deviceReserved) {
          await admin.from('users').update({ free_minute_used_at: new Date().toISOString() })
            .eq('id', user.id).is('free_minute_used_at', null);
        }
